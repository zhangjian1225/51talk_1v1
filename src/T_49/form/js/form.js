    ...
    methods: {
        ...
        validate: function() {
            ...
            for(var i=0;i<options.length;i++){
                if(options[i].image==''&&options[i].text==''){
                    num++
                }
                if(options[i].image!=''){
                    if(options[i].text.length>50){
                        alert("文本和图片同时存在时，文本的字数应小于等于50")
                        return 
                    }
                }
                var isNumber = reg.test(options[i].time)
                if(!isNumber||options[i].time>30){
                    wrongNum1++
                }
                if(options[i].time==''){
                    wrongNum2++
