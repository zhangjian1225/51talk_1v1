...
new Vue({
  ...
  methods: {
    ...
    validate: function() {
      ...
      var childImgNum = 0;
      var that = this;
      $.each(this.configData.source.momImg, function(item) {
        if (that.configData.source.momImg[item] != '') {
          momImgNum++;
        }
      })
      $.each(this.configData.source.childImg, function(item) {
        if (that.configData.source.childImg[item] != '') {
          childImgNum++;
        }
      })
      $.each(this.configData.source.dadImg, function(item) {
        if (that.configData.source.dadImg[item] != '') {
          dadImgNum++;
        }
      })
      if (momImgNum == 1) {
