...
new Vue({
  ...
  methods: {
    ...
    validate: function() {
      ...
      if (this.configData.source.itemName == '') {
        return false;
      }
      if (this.configData.source.text == '') {
        return false;
      }
      return true;
    },
    ...
    addSele: function(type) {
      if (type == 'list') {
        ...
        this.configData.source.bodyList.push({
          img: '',
          color: '#000'
        });
      }
    },
    delSele: function(item, type) {
      if (type == 'list') {
        this.configData.source.list.remove(item);
      } else {
        this.configData.source.bodyList.remove(item);
      }
    },
    addTg: function(item) {
