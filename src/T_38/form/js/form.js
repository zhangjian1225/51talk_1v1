...
new Vue({
  ...
  methods: {
    addCards: function() {
      if (this.configData.source.words.length >= 6) {
        return;
      }
    ...
    delCards: function(item) {
      this.configData.source.words.remove(item)
    },
    ...
    validate: function() {
			let words = this.configData.source.words; 
			let empty = words.some(function(item){
      ...
      empty = words.some(function(item){
				return item.pic == '';
			});
			if(empty){
				alert("图片项不允许为空！");
        return false;
			...
			let spec = words.some(function(item){
				return !/^[\u0000-\u00FF]+$/.test(item.word)
			});
			if(spec){
				alert("只能填写英文！");
        return false;
