...
new Vue({
  ...
  methods: {
    addWordOption: function() {
      this.configData.source.words.push("");
    },
    delWordOption: function(index) {
      this.configData.source.words.splice(index, 1);
    },
    ...
    validate: function() {
      var isPass = this.configData.source.words.some(function(item){
          return item == ''
      });
      if(isPass == true){
          alert('带*为必填项');
      ...
      if(this.configData.source.repeats<1||this.configData.source.repeats>20){
        alert('重复次数必须在 1~20 之间');
        return false;
      }
      return true;
    },
