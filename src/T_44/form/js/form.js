...
new Vue({
    ...
    methods: {
        ...
        validate: function() {
            if (this.configData.source.type == "img") {
                var isPass = this.configData.source.trainMsg.some(function(item){
                    return item.img == ''
                });
                var isPass2 = this.configData.source.distracterList.some(function(item){
                    return item.img == ''
                });
                if(isPass == true || isPass2 == true){
                    return false;
                }
                return true;
            } else {
        ...
        onSend: function() {
            ...
            var num = 0;
            var totleNum = this.configData.source.distracterList.length;
            this.configData.source.trainMsg.some(function(item){
            ...
            if (val === true) {
                if (num<1) {
                    ...
                    if (num + totleNum >6) {
                        alert("车厢至默认为空和干扰项的个数最多只可以有6项请重新编辑");
                    } else {
        ...
        addSele: function() {
            this.configData.source.trainMsg.push({
                    isEmpty: 'true',
                    pos:'',
                    img:'',
                    font: ''
        ...
        delSele: function(item) {
            this.configData.source.trainMsg.remove(item);
        },
        addDis: function() {
            this.configData.source.distracterList.push({
                    pos:'#',
                    img:'',
                    font: ''
            });
        },
        delDis: function (item) {
            this.configData.source.distracterList.remove(item);
        },
		addTg:function(item){
        ...
        isType: function (type) {
            this.configData.source.type = type;
        },
        isEmpty: function (item, type) {
            item.isEmpty = type;
        }
