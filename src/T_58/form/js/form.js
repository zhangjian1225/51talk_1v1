...
new Vue({
  ...
  methods: {
    ...
    validate: function () {
      if (this.configData.source.doorImg && this.configData.source.font) {
        return true;
      } else {
        return false;
      }
    ...
    onSend: function () {
      ...
      var reg = /^[1-9]\d*$/;
      var isNumber = reg.test(data.source.num)
      if (!isNumber || data.source.num > 5) {
        alert('请选择1~5范围的正整数')
        return
      }
      if (val) {
        var _data = JSON.stringify(data);
        if (this.configData.source.doorBg || this.configData.source.lockOne || this.configData.source.lockTwo || this.configData.source.lockThree) {
          if (this.configData.source.doorBg && this.configData.source.lockOne && this.configData.source.lockTwo && this.configData.source.lockThree) {
            $.ajax({
              url: domain + 'content?_method=put',
              type: 'POST',
            ...
            alert('环境素材仅允许全部上传替换，或者全不替换；不允许仅上传一部分。')
          }
        } else {
          $.ajax({
            url: domain + 'content?_method=put',
