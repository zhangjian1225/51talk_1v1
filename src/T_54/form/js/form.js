...
new Vue({
  ...
  methods: {
    ...
    validate: function() {
      let that = this;
      let id = 0;
      that.configData.source.addList = [];
      that.configData.source.ranList = [];
      // 添加两次
      this.configData.source.initList.some(function(item,index){
        id = index;
        that.configData.source.addList.push({
          id:item.id,
          img: item.img,
          font: item.font
        });
      })
      this.configData.source.initList.some(function(item){
        id ++;
        item.id = id
        that.configData.source.addList.push({
          id:id,
      ...
      this.configData.source.addList.some(function(item){
        that.configData.source.ranList.push(item);
      })
      var isPass = this.configData.source.initList.some(function(item){
        return item.img == '';
      });
      if(isPass == true || this.configData.source.time == ''){
          return false;
      }
      return true;
    },
    onSend: function() {
      ...
      this.configData.source.ranList.sort(function() {
        return Math.random() - 0.5
      })
      this.configData.source.addList.sort(function() {
        return Math.random() - 0.5
      })
      if (val) {
    ...
    addCards: function () {
      this.configData.source.initList.push({
          id:'',
          img: '',
          font: ''
        })
    ...
    deleCards: function (item) {
      this.configData.source.initList.remove(item);
    }
  }
});
