...
$.ajax({
  ...
  success: function(res) {
    if (res.data != "") {
      ...
      if (!Data.configData.source.seleListe[0].font) {
        for (var i = 0; i < Data.configData.source.seleListe.length; i++) {
          Data.configData.source.seleListep[i].font = ''
        }
      }
...
new Vue({
  ...
  methods: {
    ...
    validate: function() {
      var isPass = this.configData.source.seleList.some(function(item) {
    ...
    onSend: function() {
      ...
      // var isFont = 0;
      //提交前重新给Key复制,判断是否填写单词
      for (var i = 0, length = data.source.seleList.length; i < length; i++) {
        data.source.seleList[i].key = i + 1;
        /* if(data.source.seleList[i].font){
            isFont++
        } */
      }
      data.source.random = Math.round(Math.random() * 100);
      var _data = JSON.stringify(data);
      // var dataCounts = data.source.seleList.length;
      if (val === true) {
        // var _data = JSON.stringify(data);
        $.ajax({
          url: domain + 'content?_method=put',
