...
new Vue({
  ...
  methods: {
    ...
    validate: function() {
      if (this.configData.source.pos_one[1] < this.configData.source.pos_one[0]) {
        alert("左边猴子起始位置不可以大于终止位置");
        return false;
      }
      if (this.configData.source.pos_two[1] < this.configData.source.pos_two[0]) {
        alert("右边边猴子起始位置不可以大于终止位置");
        return false;
      }
      return true;
    },
