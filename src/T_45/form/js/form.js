...
new Vue({
  ...
  methods: {
    ...
    validate: function() {
      var isPass = this.configData.source.cards.some(function(item){
          return item.pic == '' || item.text == ''
      });
      if(isPass == true){
          alert('带*为必填项')
          return false;
      ...
      isPass = this.configData.source.thief.some(function(item){
        return item.pic == ''
      });
      if(isPass == true){
          alert('请上传盗窃／归还者素材');
    ...
    addCards: function() {
      if (this.configData.source.cards.length >= 6) {
        return;
      }
      this.configData.source.cards.push({
        pic: '',
        text: ''
      })
    },
    delCards: function(item) {
      this.configData.source.cards.remove(item)
    },
    addThiefs: function() {
      if (this.configData.source.thief.length >= 2) {
        return;
      }
      this.configData.source.thief.push({pic:''})
    },
    delThiefs: function(item) {
      this.configData.source.thief.remove(item)
    },
